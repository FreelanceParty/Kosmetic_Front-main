export const getUserFirstName = (state) => state.auth.firstName;
export const getUserLastName = (state) => state.auth.lastName;
export const getUserNumber = (state) => state.auth.number;
export const getUserEmail = (state) => state.auth.email;
export const getUserToken = (state) => state.auth.token;
export const getIsLoggedIn = (state) => state.auth.isLoggedIn;
export const getIsRefreshing = (state) => state.auth.isRefreshing;
export const getIsRegister = (state) => state.auth.isRegister;
export const getIsAdmin = (state) => state.auth.isAdmin;
export const getOptUser = (state) => state.auth.optUser;
export const getDropUser = (state) => state.auth.dropUser;
export const getUserId = (state) => state.auth._id;
export const getOwnedCabinets = (state) => state.auth.ownedCabinets;
export const getActiveCabinet = (state) => state.auth.activeCabinet;
export const getUserCity = (state) => state.auth.city;
export const getUserLink = (state) => state.auth.link;
export const getUserOnlineShop = (state) => state.auth.onlineShop;
export const getUserOfflineShop = (state) => state.auth.offlineShop;
export const getUserSocialMedia = (state) => state.auth.socialMedia;
